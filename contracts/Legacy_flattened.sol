...
contract Legacy is ReentrancyGuard, AutomationCompatibleInterface {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    event EthSubscriptionFeeTransfered(
        address sender,
        address receiver,
        uint256 amount,
        uint256 subscriptionID,
    ...
    event StableCoinSubscriptionFeeTransfered(
        address sender,
        address receiver,
        address token,
        uint256 amount,
    ...
    address public subscriptionFeeReceiver;
    address public gasFeeReceiver;
    uint256 public gasFeePercent = 3; // in dollars

    // fee of activation
    uint public feeInStableCoin = 369 * 1e18; // $369 in stablecoin (like USDT)
    uint public graceFeeIncrement = 3690 * 1e16; // 36.90 stablecoin units
    // each period time limit
    uint public subscriptionPeriodTimeLimit = 369 days; // Subscription period in seconds
    uint public gracePeriodTimeLimit = 36 days + 9 hours; // time period for 1st grace period.
    ...
    struct Subscription {
        ...
        EnumerableSet.AddressSet depositedTokens;
    }
    ...
    constructor(
        ...
        address _subscriptionFeeReceiver,
        address _gasFeeReceiver,
        address[] memory _allowedStableCoins
    ) {
        owner = payable(msg.sender);

        subscriptionFeeReceiver = _subscriptionFeeReceiver;
    ...
    function activateSubscription(
        ...
        if (payInETH) {
            // Payment in ETH
            uint requiredETHFee = calculateETHFee();
            require(
                msg.value >= (requiredETHFee + _ETHDeposited),
                "Incorrect ETH subscription fee"
            );

            // Return extra eth back to the user
            if (msg.value > (requiredETHFee + _ETHDeposited)) {
                payable(msgSender).transfer(
                    msg.value - (requiredETHFee + _ETHDeposited)
                );
            }

            uint256 gasFee = (requiredETHFee * gasFeePercent) / 100;
            payable(gasFeeReceiver).transfer(gasFee);
            ...
            payable(subscriptionFeeReceiver).transfer(requiredETHFee - gasFee);
            emit EthSubscriptionFeeTransfered(
                msgSender,
                subscriptionFeeReceiver,
                requiredETHFee - gasFee,
                subscriptionCounter,
                block.timestamp
            );
        } else {
            ...
            uint adjustedFeeInStableCoin = feeInStableCoin /
                (10 ** (18 - decimals));
            uint256 gasFee = (adjustedFeeInStableCoin * gasFeePercent) / 100;

            // Transfer the gas fee in stablecoin
            require(
            ...
            // Transfer the remaining subscription fee in stablecoin
            require(
                IERC20(_stableCoin).transferFrom(
                    msgSender,
                    subscriptionFeeReceiver,
                    adjustedFeeInStableCoin - gasFee
                ),
                "Stablecoin transfer failed for subscription fee"
            );
            emit StableCoinSubscriptionFeeTransfered(
                msgSender,
                subscriptionFeeReceiver,
                _stableCoin,
                adjustedFeeInStableCoin - gasFee,
    ...
    function renewSubscription(
        ...
        if (payInETH) {
            uint256 requiredFee = calculateETHFee();
            ...
            uint256 gasFee = (requiredFee * gasFeePercent) / 100;
            payable(gasFeeReceiver).transfer(gasFee);
            emit EthGasFeeTransfered(
                msgSender,
                gasFeeReceiver,
            ...
            payable(subscriptionFeeReceiver).transfer(requiredFee - gasFee);
            emit EthSubscriptionFeeTransfered(
                msgSender,
                subscriptionFeeReceiver,
                requiredFee - gasFee,
        ...
        sub.activationTime = block.timestamp;
    ...
    // Change heir
    function changeHeir(uint256 _subscriptionId, address _newHeir) external {
        address msgSender = msg.sender;
    ...
    // Handle end of grace period and send funds to heir if no payment is made
    // function handleGracePeriodExpiration() public {
    //     Subscription storage user = subscribers[msg.sender];
    //     require(user.gracePeriod == 4, "Grace period not expired yet");

    //     uint balanceMinusGasFees = user.balance - tx.gasprice;
    //     user.heir.transfer(balanceMinusGasFees); // Transfer remaining funds to heir
    // }
    ...
    // Admin functions to set subscription period
    function setsubscriptionPeriodTimeLimit(
        uint _subscriptionPeriodTimeLimit
    ) public onlyOwner {
        subscriptionPeriodTimeLimit = _subscriptionPeriodTimeLimit;
    }

    // Admin function to set grace period time limit
    function setGracePeriodTimeLimit(
        uint _gracePeriodTimeLimit
    ) public onlyOwner {
        gracePeriodTimeLimit = _gracePeriodTimeLimit;
    }

    // Admin function to set base fee in USD
    function setBaseFeeUSD(uint _baseFeeUSD) public onlyOwner {
        feeInStableCoin = _baseFeeUSD;
    }
    ...
    function checkUpkeep(
        ...
        for (uint i = 0; i < activeIdsOfSystem.length(); i++) {
            ...
            if (
                block.timestamp >=
                sub.activationTime +
                    subscriptionPeriodTimeLimit +
                    gracePeriodTimeLimit &&
                idStatus[_id]
            ) {
    ...
    function calculateETHFee() public view returns (uint) {
        int ethPriceInUSD = getLatestPrice(); // Price of 1 ETH in USD (with 8 decimals)
        require(ethPriceInUSD > 0, "Invalid price from oracle");

        uint ethFee = (feeInStableCoin * 1e8) / uint(ethPriceInUSD); // Conversion to ETH amount
        return ethFee;
    }
        ...
        )
    {
        ...
        return (
            ...
            subscription.activationTime + subscriptionPeriodTimeLimit,
